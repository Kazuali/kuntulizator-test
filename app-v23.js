...
function calculateHistoricalPositionStats() {
  ...
  const index = Object.fromEntries(rows.map(r => [r.id, r]));
  const byId = Object.fromEntries(rows.map(r => [r.id, { participantId: r.id, name: r.name, leaderCount: 0, topZoneCount: 0, riskZoneCount: 0 }]));
  ...
  for (const b of breakdowns) {
    ...
    const snapshot = sortedActiveRows();
    if (!snapshot.length) continue;
    const leaderTotal = snapshot[0].total;
    snapshot.filter(r => Math.abs(r.total - leaderTotal) < 0.000001).forEach(r => byId[r.id].leaderCount += 1);
    snapshot.slice(0, topLimit).forEach(r => byId[r.id].topZoneCount += 1);
    snapshot.slice(topLimit).forEach(r => byId[r.id].riskZoneCount += 1);
  }
...
function renderAnalytics() {
  ...
  if (!a.completed) { byId("topAnalytics").innerHTML = empty; byId("matchAnalytics").innerHTML = empty; byId("historicalAnalytics").innerHTML = empty; }
    ...
    byId("historicalAnalytics").innerHTML = `<div class="insight-list"><div class="insight"><span>Чаще всех был лидером</span><strong>${formatHistoryWinner(a.positionHistory.leaderRows, "leaderCount")}</strong></div><div class="insight"><span>Чаще всех был в топ-${getTopPlaces()}</span><strong>${formatHistoryWinner(a.positionHistory.topZoneRows, "topZoneCount")}</strong></div><div class="insight"><span>Чаще всех был в ебездалах</span><strong>${formatHistoryWinner(a.positionHistory.riskZoneRows, "riskZoneCount")}</strong></div><div class="insight"><span>Контрольных точек</span><strong>${a.positionHistory.checkpoints} матчей</strong></div></div>`;
  }
  const participantRows = calculateStandings();
  const historyById = a.positionHistory.byId || {};
  byId("participantStatsTable").innerHTML = `<thead><tr><th>Участник</th><th>Очки</th><th>Лидер</th><th>Топ-${getTopPlaces()}</th><th>Ебездалы</th><th>Исходы</th><th>Точные</th><th>Мимо</th><th>Средние очки</th><th>Точность исходов</th></tr></thead><tbody>${participantRows.map(r => { const h = historyById[r.id] || {}; return `<tr><td>${e(r.name)}</td><td>${fmt(r.total)}</td><td>${h.leaderCount || 0}</td><td>${h.topZoneCount || 0}</td><td>${h.riskZoneCount || 0}</td><td>${r.resultHits}</td><td>${r.exactHits}</td><td>${r.misses}</td><td>${r.played ? fmt(r.total / r.played) : "—"}</td><td>${pct(r.resultHits, r.played)}</td></tr>`; }).join("")}</tbody>`;
}
