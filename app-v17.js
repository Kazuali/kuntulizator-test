...
function init() {
  initTabs(); initSubmitHandlers(); initAdminChat(); renderAll();
  const button = byId("refreshButton");
...
function initAdminChat() {
  ...
  let replyIndex = Number(localStorage.getItem("kuntulizatorAdminReplyIndexV17") || "0") || 0;
  ...
  function openChat() {
    const phone = overlay.querySelector(".nokia-chat__phone");
    overlay.classList.add("is-open");
    overlay.setAttribute("aria-hidden", "false");
    if (phone) {
      phone.classList.remove("nokia-chat__phone--opened");
      phone.classList.add("nokia-chat__phone--closed");
      window.setTimeout(() => {
        phone.classList.remove("nokia-chat__phone--closed");
        phone.classList.add("nokia-chat__phone--opened");
      }, 180);
    }
    window.setTimeout(() => input.focus(), 1050);
  }
  function closeChat() {
    const phone = overlay.querySelector(".nokia-chat__phone");
  ...
  form.addEventListener("submit", (event) => {
    ...
    typingTimer = window.setTimeout(() => {
      ...
      replyIndex += 1;
      localStorage.setItem("kuntulizatorAdminReplyIndexV17", String(replyIndex));
    }, 1050);
  });
}
