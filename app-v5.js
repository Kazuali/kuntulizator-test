...
function renderZonePreview() {
  ...
  byId("zonePreview").innerHTML = `<div class="table-wrap"><table><thead><tr><th>Место</th><th>Участник</th><th>Очки</th><th>Зона</th></tr></thead><tbody>${standings.map((r, i) => {
    ...
    return `<tr><td class="rank">${i + 1}</td><td>${e(r.name)}</td><td>${fmt(r.total)}</td><td class="${good ? "zone-good" : "zone-bad"}">${good ? `Топ-${data.settings.topPlaces}` : "Ебездалы"}</td></tr>`;
...
function sendViaJsonp(payload) {
  ...
  if (!url || url.includes("PASTE_GOOGLE_APPS_SCRIPT")) {
    return Promise.reject(new Error("Сначала нужно вставить ссылку Google Apps Script Web App в data-v5.js."));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    window[callbackName] = (response) => { cleanup(); response && response.ok ? resolve(response) : reject(new Error(response?.message || "Не удалось сохранить прогнозы.")); };
    script.onerror = () => { cleanup(); reject(new Error("Не удалось связаться с Google Apps Script.")); };
    const separator = url.includes("?") ? "&" : "?";
    script.src = `${url}${separator}action=submitPredictions&callback=${encodeURIComponent(callbackName)}&payload=${encodeURIComponent(JSON.stringify(payload))}&_=${Date.now()}`;
...
function initSubmitHandlers() {
  ...
  const demoButton = byId("fillDemoButton");
  if (demoButton) demoButton.addEventListener("click", () => {
    document.querySelectorAll(".submit-match-row").forEach((row, index) => {
      row.querySelector(".home-score").value = index % 4;
      row.querySelector(".away-score").value = (index + 1) % 3;
    });
  });
}
