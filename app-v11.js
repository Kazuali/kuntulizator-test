...
function renderPredictionControls() {
  ...
  select.innerHTML = data.matches.map(m => `<option value="${e(m.id)}">${e(m.round)} · ${e(matchTitle(m))}</option>`).join("");
...
function renderSubmitMatchRowsForParticipant(participantNameValue) {
  ...
  container.innerHTML = matches.map((m) => {
    ...
    return `<div class="submit-match-row" data-round-key="${e(currentSubmitRoundKey)}" data-match-id="${e(m.id)}" data-sheet-column="${e(m.sheetColumn || index + 2)}" data-match-title="${e(matchTitle(m))}"><div class="submit-match-title">${displayIndex}. ${e(matchTitle(m))}</div><input class="score-input home-score" type="number" inputmode="numeric" min="0" max="20" placeholder="0"><span class="score-separator">-</span><input class="score-input away-score" type="number" inputmode="numeric" min="0" max="20" placeholder="0"></div>`;
  }).join("");
}
...
function renderSubmitForm() {
  ...
  if (participantSelect) {
    ...
    renderSubmitMatchRowsForParticipant(participantSelect.value || activeParticipants[0]?.name || "");
  }
}
...
function collectPredictionsForReview() {
  ...
  const predictions = rows.map(row => {
    ...
    const home = Number(homeRaw), away = Number(awayRaw);
...
function callSubmissionApi(action, payload) {
  ...
  if (!url || url.includes("PASTE_GOOGLE_APPS_SCRIPT")) return Promise.reject(new Error("Сначала нужно вставить ссылку Google Apps Script Web App в data-v10.js."));
...
function initSubmitHandlers() {
  ...
  if (reviewButton) reviewButton.addEventListener("click", () => { try { setSubmitMessage(""); showReview(collectPredictionsForReview()); } catch (error) { setSubmitMessage(error.message, "is-error"); } });
  ...
  if (loadButton) loadButton.addEventListener("click", async () => {
    try {
      setSubmitMessage(""); loadButton.disabled = true; loadButton.classList.add("is-loading"); loadButton.textContent = "Загружаю…";
    ...
    finally { loadButton.disabled = false; loadButton.classList.remove("is-loading"); loadButton.textContent = "Загрузить мой прогноз"; }
  });
}
...
function init() {
  initTabs(); initSubmitHandlers(); renderAll();
  const button = byId("refreshButton");
