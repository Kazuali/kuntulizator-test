...
function normalizeTeamText(value) {
  return normName(value)
    .replace(/^(проходит|победитель|победила|победил|побеждает|выиграла|выиграл)\s+/i, "")
    .replace(/\s+(проходит|победитель|победила|победил|побеждает|выиграла|выиграл)$/i, "")
    .trim();
}
function teamCodeFromText(value, match) {
  const text = normalizeTeamText(value);
  if (!text || !match) return null;
  const home = normalizeTeamText(match.home);
  const away = normalizeTeamText(match.away);
  if (text === home) return "home";
  if (text === away) return "away";
  return null;
}
function winnerCodeFromScore(home, away, explicitWinner = null) {
  if (home > away) return "home";
  if (home < away) return "away";
  return explicitWinner || null;
}
function parseScoreWithWinner(value, match) {
  const raw = clean(value);
  if (!raw) return null;
  const scoreMatch = raw.match(/^(\d{1,2})\s*[-:.,—–]\s*(\d{1,2})(?:\s+(.+))?$/u);
  if (!scoreMatch) return null;
  const home = Number(scoreMatch[1]);
  const away = Number(scoreMatch[2]);
  const explicitWinner = teamCodeFromText(scoreMatch[3] || "", match);
  return { home, away, winner: winnerCodeFromScore(home, away, explicitWinner) };
}
function roundByKey(key) { return (data.settings.rounds || []).find(r => r.key === key) || null; }
function isPlayoffRoundKey(roundKey) { return (roundByKey(roundKey)?.type || "") === "playoff"; }
function isPlayoffMatch(match) { return !!match && isPlayoffRoundKey(match.roundKey); }
function getWinnerLabel(code, match) { if (code === "home") return match?.home || "первая команда"; if (code === "away") return match?.away || "вторая команда"; return "—"; }
function formatScoreText(home, away) { return home === null || away === null ? "—" : `${home}-${away}`; }
function formatPredictionText(prediction, match, alwaysShowWinner = true) {
  if (!prediction) return "—";
  const score = formatScoreText(prediction.home, prediction.away);
  if (!isPlayoffMatch(match)) return score;
  const winner = prediction.winner || winnerCodeFromScore(prediction.home, prediction.away, null);
  if (!alwaysShowWinner || !winner) return score;
  return `${score} ${getWinnerLabel(winner, match)}`;
}
function formatMatchResult(match) {
  if (!match || match.homeScore === null || match.awayScore === null) return "—";
  const score = formatScoreText(match.homeScore, match.awayScore);
  if (!isPlayoffMatch(match)) return score;
  const winner = match.winner || winnerCodeFromScore(match.homeScore, match.awayScore, null);
  if (!winner) return `${score} · нужен победитель`;
  return `${score} · прошла ${getWinnerLabel(winner, match)}`;
}
function isMatchScorable(match) {
  if (!match || match.homeScore === null || match.awayScore === null) return false;
  if (!isPlayoffMatch(match)) return true;
  return !!(match.winner || winnerCodeFromScore(match.homeScore, match.awayScore, null));
}
function predictionResultCode(prediction, match) {
  if (!prediction || !match) return null;
  if (!isPlayoffMatch(match)) return outcome(prediction.home, prediction.away);
  return prediction.winner || winnerCodeFromScore(prediction.home, prediction.away, null);
}
function actualResultCode(match) {
  if (!match) return null;
  if (!isPlayoffMatch(match)) return outcome(match.homeScore, match.awayScore);
  return match.winner || winnerCodeFromScore(match.homeScore, match.awayScore, null);
}
function isResultHit(prediction, match) {
  const actual = actualResultCode(match);
  const predicted = predictionResultCode(prediction, match);
  return !!actual && !!predicted && actual === predicted;
}
function isExactHit(prediction, match) {
  if (!prediction || !match) return false;
  if (prediction.home !== match.homeScore || prediction.away !== match.awayScore) return false;
  if (!isPlayoffMatch(match)) return true;
  return isResultHit(prediction, match);
}
...
function getSubmissionMatchesForParticipant(name, roundKey) {
  const round = roundByKey(roundKey);
  const startIndex = Math.max(1, Number(round?.submissionStartMatchIndex || 1));
  return getRoundMatches(roundKey).filter((m, index) => {
    if (!isParticipantActiveForMatch(name, m)) return false;
    if (index + 1 < startIndex) return false;
    return true;
  });
}
...
function outcomeLabel(code, match) {
  if (isPlayoffMatch(match)) return code === "home" || code === "away" ? `Проходит: ${getWinnerLabel(code, match)}` : "Победитель не выбран";
...
function getRoundBank(roundKey) {
  ...
  const resultStake = Number(round.resultStakePerActiveParticipant ?? data.settings.resultStakePerActiveParticipant ?? 100);
  const exactStake = Number(round.exactStakePerActiveParticipant ?? data.settings.exactStakePerActiveParticipant ?? 50);
  const resultBank = data.settings.dynamicBank ? active * resultStake : (round.resultBank ?? data.settings.resultBank ?? 1400);
  const exactBank = data.settings.dynamicBank ? active * exactStake : (round.exactScoreBank ?? data.settings.exactScoreBank ?? 700);
  return { active, resultBank, exactBank, matchBank: resultBank + exactBank, resultStake, exactStake };
}
function getMatchBank(match) {
  const round = roundByKey(match?.roundKey) || {};
...
function buildRoundDataFromSheet(csvText, round) {
  ...
  for (let col = 1; col < headerRow.length; col += 1) {
    ...
    const matchLike = { home, away, roundKey: round.key };
    const parsedResult = isPlayoffRoundKey(round.key) ? parseScoreWithWinner(resultRow[col], matchLike) : parseScore(resultRow[col]);
    matchCols.push({ col, sheetColumn: col + 1, title, home, away, score: parsedResult });
  }
  ...
  const matches = matchCols.map((m, index) => {
    const isPlayoff = isPlayoffRoundKey(round.key);
    const winner = m.score ? (isPlayoff ? m.score.winner : null) : null;
    return {
      ...
      winner,
      status: m.score ? (isPlayoff && m.score.home === m.score.away && !winner ? "needs_winner" : "finished") : "upcoming"
    };
  });
  ...
  participantRows.forEach((row) => {
    ...
    matchCols.forEach((m, matchIndex) => {
      const matchLike = { home: m.home, away: m.away, roundKey: round.key };
      const score = isPlayoffRoundKey(round.key) ? parseScoreWithWinner(row[m.col], matchLike) : parseScore(row[m.col]);
      if (!score) return;
      predictions.push({
        ...
        home: score.home,
        away: score.away,
        winner: isPlayoffRoundKey(round.key) ? score.winner : null
      });
    });
  });
...
function calculateMatchBreakdowns() {
  return data.matches.filter(match => isMatchScorable(match)).map(match => {
    const fact = actualResultCode(match);
    ...
    const resultWinners = preds.filter(p => isResultHit(p, match));
    const exactWinners = preds.filter(p => isExactHit(p, match));
    ...
    const rows = preds.map(p => {
      ...
      return {
        ...
        prediction: formatPredictionText(p, match),
    ...
    const roundDiff = Number(a.match.roundOrder || 0) - Number(b.match.roundOrder || 0);
    if (roundDiff) return roundDiff;
    const aCol = Number(a.match.sheetColumn || 0);
    const bCol = Number(b.match.sheetColumn || 0);
    if (aCol && bCol && aCol !== bCol) return aCol - bCol;
    const aNum = Number(String(a.match.id || "").match(/_m(\d+)$/)?.[1] || 0);
    const bNum = Number(String(b.match.id || "").match(/_m(\d+)$/)?.[1] || 0);
    return aNum - bNum;
  });
}
...
function calculateHistoricalPositionStats() {
  ...
  for (let i = 0; i < breakdowns.length; i += 1) {
    ...
    checkpoints.push({
      ...
      result: formatMatchResult(b.match),
...
function getAnalytics() {
  ...
  let bestSingle = null;
  for (const b of breakdowns) for (const row of b.rows) if (!bestSingle || row.total > bestSingle.total) bestSingle = { ...row, matchTitle: matchTitle(b.match), score: formatMatchResult(b.match) };
...
function renderHeroMeta() {
  ...
  if (subtitle) subtitle.textContent = `${data.participants.length} участников, ${data.matches.length} матчей. Группа: 100+50 с участника, плей-офф: 100+100 с участника.`;
...
function renderUpcoming() {
  ...
  byId("upcomingMatches").innerHTML = list.map(m => `<div class="match-card"><h3>${e(matchTitle(m))}</h3><p class="muted">${e(m.round)}</p><div class="score">${m.homeScore === null ? "результат пока не введён" : formatMatchResult(m)}</div></div>`).join("");
}
...
function renderMatches() {
  ...
  byId("matchCards").innerHTML = visibleMatches.map((m, index) => {
    ...
    const active = getActiveParticipantsForMatch(m).length;
    ...
    return `<div class="match-card"><h3>${index + 1}. ${e(matchTitle(m))}</h3><p class="muted">${e(m.round)}</p><div class="score">${formatMatchResult(m)}</div>${meta}</div>`;
...
function renderHiddenPredictionStatus(match) {
  ...
  byId("predictionSummary").innerHTML = `<div class="lock-note"><strong>Прогнозы скрыты до дедлайна.</strong><br>${e(match.round)}: видно только, кто сдал тур. Дедлайн: ${e(formatDeadline(match.roundKey))}. Сдали: ${submitted}/${active.length}.</div>`;
...
function renderPredictions(matchId) {
  ...
  const active = getActiveParticipantsForMatch(match);
  ...
  const homeCount = preds.filter(p => predictionResultCode(p, match) === "home").length;
  const drawCount = isPlayoffMatch(match) ? 0 : preds.filter(p => predictionResultCode(p, match) === "draw").length;
  const awayCount = preds.filter(p => predictionResultCode(p, match) === "away").length;
  byId("predictionSummary").innerHTML = isPlayoffMatch(match)
    ? `<div class="summary-pill"><span>${e(match.home)}</span><strong>${homeCount}</strong><small>прогнозов на проход</small></div><div class="summary-pill"><span>${e(match.away)}</span><strong>${awayCount}</strong><small>прогнозов на проход</small></div>`
    : `<div class="summary-pill"><span>${e(match.home)}</span><strong>${homeCount}</strong><small>прогнозов на победу</small></div><div class="summary-pill"><span>Ничья</span><strong>${drawCount}</strong><small>прогнозов</small></div><div class="summary-pill"><span>${e(match.away)}</span><strong>${awayCount}</strong><small>прогнозов на победу</small></div>`;
  byId("predictionsTable").innerHTML = `<thead><tr><th>Участник</th><th>Прогноз</th><th>${isPlayoffMatch(match) ? "Проход" : "Исход прогноза"}</th><th>Очки за матч</th></tr></thead><tbody>${active.map(participant => {
    const p = preds.find(x => x.participantId === participant.id);
    if (!p) return `<tr><td>${e(participant.name)}</td><td>—</td><td>не сдал</td><td>—</td></tr>`;
    const o = predictionResultCode(p, match);
    const points = pointsByParticipant[p.participantId]?.total;
    return `<tr><td>${e(p.name)}</td><td>${e(formatPredictionText(p, match))}</td><td>${e(outcomeLabel(o, match))}</td><td>${points === undefined ? "—" : fmt(points)}</td></tr>`;
  }).join("")}</tbody>`;
}


function ensureAnalyticsAuditBlocks() {
  const analytics = byId("analytics");
  if (!analytics) return;
  const statsTable = byId("participantStatsTable");
  const statsCard = statsTable ? statsTable.closest(".card") : null;
  let anchor = statsCard || analytics.lastElementChild;

  if (!byId("roundAuditTable")) {
    const card = document.createElement("article");
    card.className = "card analytics-table-card audit-card injected-audit-card";
    card.innerHTML = `<h2>Итоги после каждого тура</h2><p class="muted">Здесь видно, сколько очков было у каждого участника после последнего завершённого матча каждого тура.</p><div class="table-wrap"><table id="roundAuditTable"></table></div>`;
    if (anchor && anchor.parentNode === analytics) {
    ...
    const existing = byId("roundAuditTable").closest(".card");
    if (existing) anchor = existing;
  }

  if (!byId("positionAuditTable")) {
    const card = document.createElement("article");
    card.className = "card analytics-table-card audit-card injected-audit-card";
    card.innerHTML = `<h2>Проверка истории после каждого матча</h2><p class="muted">Расшифровка для проверки: кто был лидером, кто был последним и какая была полная таблица после каждого завершённого матча.</p><div class="table-wrap"><table id="positionAuditTable"></table></div>`;
    if (anchor && anchor.parentNode === analytics) {
      anchor.insertAdjacentElement("afterend", card);
    } else {
      analytics.appendChild(card);
...
function updatePlayoffWinnerRow(row) {
  if (!row || row.dataset.playoff !== "true") return;
  const homeInput = row.querySelector(".home-score");
  const awayInput = row.querySelector(".away-score");
  const buttons = Array.from(row.querySelectorAll(".winner-button"));
  ...
  if (Number.isInteger(home) && Number.isInteger(away)) {
    if (home > away) forced = "home";
    else if (home < away) forced = "away";
  }
  if (forced) row.dataset.winner = forced;
  buttons.forEach(button => {
    const code = button.dataset.winner;
    const selected = row.dataset.winner === code;
    button.classList.toggle("is-selected", selected);
    button.disabled = !!forced && code !== forced;
    button.setAttribute("aria-pressed", selected ? "true" : "false");
  });
  const hint = row.querySelector(".winner-hint");
  if (hint) {
    if (forced) hint.textContent = `Победитель выбран автоматически по счёту: ${forced === "home" ? row.dataset.homeTeam : row.dataset.awayTeam}`;
    else hint.textContent = "При ничьей выбери, кто проходит дальше.";
  }
}
function bindSubmitRowEvents() {
  document.querySelectorAll(".submit-match-row").forEach(row => {
    row.querySelectorAll(".home-score, .away-score").forEach(input => input.addEventListener("input", () => updatePlayoffWinnerRow(row)));
    row.querySelectorAll(".winner-button").forEach(button => {
      button.addEventListener("click", () => {
        const home = Number(row.querySelector(".home-score")?.value);
        const away = Number(row.querySelector(".away-score")?.value);
        if (Number.isInteger(home) && Number.isInteger(away) && home !== away) {
          updatePlayoffWinnerRow(row);
          return;
        }
        row.dataset.winner = button.dataset.winner;
        updatePlayoffWinnerRow(row);
      });
    });
    updatePlayoffWinnerRow(row);
...
function renderSubmitMatchRowsForParticipant(participantNameValue) {
  ...
  container.innerHTML = matches.map((m) => {
    ...
    const isPlayoff = isPlayoffMatch(m);
    const winnerControls = isPlayoff ? `<div class="winner-choice" role="group" aria-label="Кто проходит дальше"><span class="winner-choice-label">Кто проходит:</span><button class="winner-button" type="button" data-winner="home">${e(m.home)}</button><button class="winner-button" type="button" data-winner="away">${e(m.away)}</button><small class="winner-hint">При ничьей выбери, кто проходит дальше.</small></div>` : "";
    return `<div class="submit-match-row ${isPlayoff ? "submit-match-row--playoff" : ""}" data-round-key="${e(currentSubmitRoundKey)}" data-match-id="${e(m.id)}" data-sheet-column="${e(m.sheetColumn || index + 2)}" data-match-title="${e(matchTitle(m))}" data-playoff="${isPlayoff ? "true" : "false"}" data-home-team="${e(m.home)}" data-away-team="${e(m.away)}"><div class="submit-match-title">${displayIndex}. ${e(matchTitle(m))}</div><input class="score-input home-score" type="number" inputmode="numeric" min="0" max="20" value="0"><span class="score-separator">-</span><input class="score-input away-score" type="number" inputmode="numeric" min="0" max="20" value="0">${winnerControls}</div>`;
  }).join("");
  bindSubmitRowEvents();
}
...
function renderSubmitForm() {
  ...
  if (closed) {
    ...
    if (matchesBox) {
      matchesBox.innerHTML = `<div class="lock-note"><strong>Тур уже начался.</strong><br>Отправка и редактирование прогнозов закрыты. Если участник не сдал прогноз до дедлайна, по этому этапу у него остаётся пусто.</div>`;
    }
    return;
  }
...
function collectPredictionsForReview() {
  ...
  const predictions = rows.map(row => {
    ...
    let winner = null;
    if (row.dataset.playoff === "true") {
      winner = winnerCodeFromScore(home, away, row.dataset.winner || null);
      if (!winner) throw new Error(`В матче «${row.dataset.matchTitle}» при ничьей выбери, кто проходит дальше.`);
      row.dataset.winner = winner;
      updatePlayoffWinnerRow(row);
    }
    return { matchId: row.dataset.matchId, matchTitle: row.dataset.matchTitle, sheetColumn: Number(row.dataset.sheetColumn), home, away, winner };
  });
...
function showReview(payload) {
  ...
  byId("reviewList").innerHTML = payload.predictions.map((p, i) => {
    const match = getMatchById(p.matchId) || { home: "первая команда", away: "вторая команда", roundKey: payload.roundKey };
    return `<div class="review-item"><span>${i + 1}. ${e(p.matchTitle)}</span><strong>${e(formatPredictionText(p, match))}</strong></div>`;
  }).join("");
...
function callSubmissionApi(action, payload) {
  ...
  if (!url || url.includes("PASTE_GOOGLE_APPS_SCRIPT")) return Promise.reject(new Error("Сначала нужно вставить ссылку Google Apps Script Web App в data-v32.js."));
...
function applyLoadedPredictions(predictions) {
  ...
  document.querySelectorAll(".submit-match-row").forEach(row => {
    ...
    if (!p) { updatePlayoffWinnerRow(row); return; }
    ...
    if (homeInput && awayInput) {
      ...
      if (row.dataset.playoff === "true") row.dataset.winner = winnerCodeFromScore(p.home, p.away, p.winner || null) || "";
...
function initSubmitHandlers() {
  ...
  if (loadButton) loadButton.addEventListener("click", async () => {
    try {
      ...
      if (response.loaded) {
        const selectedName = clean(byId("submitParticipant")?.value);
        const expected = getSubmissionMatchesForParticipant(selectedName, currentSubmitRoundKey).length;
        setSubmitMessage(`Загружен сохранённый прогноз: ${response.loaded}/${expected} матчей. Можно исправить и отправить заново до дедлайна.`, "is-ok");
      }
