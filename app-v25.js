...
function calculateHistoricalPositionStats() {
  ...
  const index = Object.fromEntries(rows.map(r => [r.id, r]));
  const byId = Object.fromEntries(rows.map(r => [r.id, { participantId: r.id, name: r.name, leaderCount: 0, topZoneCount: 0, riskZoneCount: 0, lastPlaceCount: 0 }]));
  const checkpoints = [];
  ...
  for (const b of breakdowns) {
    ...
    checkpoints.push({
      ...
      snapshot: snapshot.map((r, i) => ({ place: i + 1, id: r.id, name: r.name, total: r.total }))
    });
  }
...
function renderAnalytics() {
  ...
  if (auditEl) {
    auditEl.innerHTML = auditRows.length
      ? `<thead><tr><th>#</th><th>Тур</th><th>Матч</th><th>Результат</th><th>Лидер после матча</th><th>Лох ебаный после матча</th><th>Ебездалы после матча</th></tr></thead><tbody>${auditRows.map(row => `<tr><td>${row.number}</td><td>${e(row.roundTitle)}</td><td>${e(row.matchTitle)}</td><td>${e(row.result)}</td><td>${row.leaders.map(x => `${e(x.name)} (${fmt(x.total)})`).join(", ")}</td><td>${row.lastPlace.map(x => `${e(x.name)} (${fmt(x.total)})`).join(", ")}</td><td>${row.riskZone.length ? row.riskZone.map(e).join(", ") : "—"}</td></tr>`).join("")}</tbody>`
